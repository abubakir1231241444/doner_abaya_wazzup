...
- Доставка от заведения отсутствует.
- Клиент может выбрать: самовывоз, в кафе или своего курьера (Яндекс/InDriver).
- Для подтверждения оплаты принимается только PDF-файл чека Kaspi (скриншоты/фото не принимаются).

## Webhook Secret

- Если `WEBHOOK_SECRET` задан в `.env`, endpoint `/webhook` принимает только запросы с `?secret=<ваш_секрет>`.
- Если `WEBHOOK_SECRET` пустой, проверка секрета отключена.
